export const getSubTasks = async ({ taskId }: any) => {
  const res = await fetch(
    `https://kupm-api2.herokuapp.com/api/tasks/${taskId}/subtasks`
  );
...
export const postTask = async (payload: any) => {
  const { id } = payload;
  const baseEndpoint = 'https://kupm-api2.herokuapp.com/api/tasks';
...
export const deleteTaskRequest = async (payload: any) => {
  const resp = await fetch(
    `https://kupm-api2.herokuapp.com/api/tasks/${payload.id}`,
    {
      method: 'DELETE',
    }
  );
